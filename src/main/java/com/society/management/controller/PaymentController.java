...
public class PaymentController {
    ...
    public Map<String,Object> add(@PathVariable long residentId,@RequestBody Map<String,Object> p,HttpSession s){
        ...
        if(!memberExists(residentId))throw new RuntimeException("Member not found");
        String contributionMonth = str(p,"contributionMonth").trim();
        // A payment must be tied to a month so that it is included in that
        // member's collected total and payment-status calculation.
        ...
        x.put("id",store.nextId("payments.json"));x.put("residentId",residentId);x.put("amount",amount);x.put("paymentDate",date);
        x.put("contributionMonth",contributionMonth);x.put("paymentMethod",str(p,"paymentMethod"));x.put("transactionId",str(p,"transactionId"));x.put("status","PAID");
    ...
    // =========================================================
    // MONTHLY PAYMENT STATUS FOR ALL MEMBERS
    // =========================================================
    @GetMapping("/status")
    public List<Map<String, Object>> monthlyStatus(
        ...
        for (Map<String, Object> member : members) {
            ...
            long memberId =
                    JsonStorageService.number(member.get("id"));

            double expected =
                    getNumber(member.get("expectedAmount"));

            Map<String, Object> latestPayment = null;
            double paid = 0;

            // Find the payment for THIS member + THIS month.
            // If multiple old records exist, use the latest one.
            for (Map<String, Object> payment : payments) {

                long paymentMemberId =
                        JsonStorageService.number(
                                payment.get("residentId"));

                String paymentMonth =
                        getString(payment, "contributionMonth");

                if (paymentMemberId == memberId
                        && month.equals(paymentMonth)) {

                    // All payments for the member and month contribute to
                    // collection and status.  Keep the newest one only for
                    // displaying the payment details in the table.
            ...
            row.put("memberName",
                    getString(member, "memberName"));
            row.put("phoneNumber",
                    getString(member, "phoneNumber"));
            row.put("email",
                    getString(member, "email"));
            row.put("flatNumber",
                    getString(member, "flatNumber"));
            row.put("month", month);
    ...
    private boolean memberExists(long id){return store.read("members.json").stream().anyMatch(x->JsonStorageService.number(x.get("id"))==id);}
    ...
    private void copy(Map<String,Object>x,Map<String,Object>p){x.put("amount",num(p.get("amount")));x.put("paymentDate",str(p,"paymentDate"));x.put("contributionMonth",str(p,"contributionMonth"));x.put("paymentMethod",str(p,"paymentMethod"));x.put("transactionId",str(p,"transactionId"));x.put("status","PAID");}

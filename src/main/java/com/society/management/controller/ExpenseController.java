...
public class ExpenseController {
    ...
    @PutMapping("/{id}") public Map<String,Object> edit(@PathVariable long id,@RequestBody Map<String,Object> p,HttpSession s){
        ...
        if(!isCurrentMonth(str(x,"expenseDate")))throw new IllegalStateException("Only current-month expenses can be updated");
    ...
    private boolean isCurrentMonth(String date){try{LocalDate expenseDate=LocalDate.parse(date);LocalDate today=LocalDate.now();return expenseDate.getYear()==today.getYear()&&expenseDate.getMonth()==today.getMonth();}catch(Exception e){return false;}}
